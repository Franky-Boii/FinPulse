...
DB_URI = os.environ.get(
    "DB_URI",
    "postgresql://finpulse:finpulse@localhost:5432/ecommerce",
)
...
def generate_email():
    """
    Generate a realistic-looking synthetic customer email address.

    Faker's default email() provider uses reserved example domains.
    We instead generate a unique username and attach a real-world
...
def place_order(cur):
    ...
    log.info(
        "Order #%s placed for customer #%s (R%.2f)",
        order_id,
        customer_id,
        total,
    )
...
def main():
    log.info(
        "Starting order generator — ~%s events/min against %s",
        EVENTS_PER_MINUTE,
        DB_URI.split("@")[-1],
    )
